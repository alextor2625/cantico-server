  ...
  try {
    ...
    if (activeSession) {
      return res.status(400).json({
        success: false,
        message:
          "No puedes crear una sesion, mientras tengas una sesion activa",
      });
    }
...
router.put("/update/:sessionId", isAuthenticated, (req, res, next) => {
  ...
  if (isActive) {
    ...
    Session.findByIdAndUpdate(sessionId, { name, isActive }, { new: true })
      .then((updatedSession) => {
        io.emit("update_session", updatedSession);
        res.status(201).json({ success: true, session: updatedSession });
      })
...
router.put("/toggle-has-started/:sessionId", async (req, res) => {
  const io = getIo();
